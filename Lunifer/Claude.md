...
- **Libre Franklin** is a variable font (`LibreFranklin-VariableFont_wght.ttf`). Do NOT use `.custom("Libre Franklin", size:).weight(.light)` — SwiftUI cannot apply weight modifiers to variable fonts via the family-name lookup and logs a warning. Always use `Font.libreFranklin(size:)` from `Utils.swift` instead. Pass `weight: 400` for Regular.
  - ⚠️ **KNOWN BUG:** the Libre Franklin `.ttf` files (`fonts/LibreFranklin-VariableFont_wght.ttf`, `LibreFranklin-Italic-VariableFont_wght.ttf`) are **NOT listed in `Info.plist`'s `UIAppFonts`** (which only registers Cormorant Garamond and DM Sans). So `Font.libreFranklin(...)`'s `UIFontDescriptor` family lookup finds no registered font and silently falls back to the system font — every hero numeric display (alarm time in `Main.swift`, added-alarm times, commute duration in `CommuteDashboard.swift`, sleep numbers in `SleepInsights.swift`) currently renders in the system font, not Libre Franklin. Fix: add both `.ttf` filenames to `UIAppFonts` in `Info.plist`.
