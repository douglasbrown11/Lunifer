...
- `CalendarChoiceScreen` (in `Screens/Survey/SurveyObjects.swift`) appears before sign-in; user picks Apple Calendar / Google Calendar / Outlook / None. Selection stored in `ContentView.preSelectedCalendar` and threaded into both `LuniferSignin` and `LuniferSurvey`.
...
- **Deletion re-auth**: `performDeletion()` in `Settings.swift` only re-authenticates **email/password** accounts. When `user.delete()` throws `requiresRecentLogin`, it presents `showReauthPasswordSheet` (a password sheet with a "Forgot password?" reset link), then re-auths via `EmailAuthProvider.credential(withEmail:password:)` and retries the delete. For Google / Apple / Microsoft-only accounts (no linked password), it shows a message telling the user to sign out and back in, then retry deletion — there is **no in-app OAuth re-auth flow**. The previous `msReauthenticate()` PKCE helper and `MicrosoftReauthCoordinator` / `generateMSVerifier()` / `generateMSChallenge(from:)` have been **removed** from `Settings.swift`. (Note: the sign-in and calendar-connect Microsoft PKCE flows still exist in `Signin.swift` and `MicrosoftCalendarService.swift`.) Azure app registration App ID: `55d084c8-c89d-4023-95c9-c20ac76a9a30`.
