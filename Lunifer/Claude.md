...
- Navigation to About (`AboutSettingsView` — Privacy Policy / Terms links)
...
| 3 | Microphone | "Allow Lunifer to access the microphone?" | Survey step 4 → 5 — requested immediately after the brief CoreMotion prompt trigger completes. The current request only obtains permission for future ambient-volume sleep detection; no audio monitor is implemented yet | `Survey.swift` `advance()` via `requestMicrophonePermission()` |
