...
- Navigation to About (`AboutSettingsView` — Privacy Policy / Terms links). These links (and the sign-in terms checkbox links in `SignInScreen.swift`) point to the Vercel-hosted website (`https://lunifer-website.vercel.app/privacy-policy.html` and `/terms.html`) — the Google-Search-Console-verified domain — NOT the Firebase Hosting copies at `lunifer-ce086.web.app`. Identical copies of both pages exist on both hosts.
