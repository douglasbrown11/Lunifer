...
| `/commute/route` | Authenticated ORS route proxy; accepts origin/destination coordinates + commute mode and returns duration seconds + route coordinates |
| `/push/register` | Store authenticated APNs device token metadata in KV |
| `/push/unregister` | Remove authenticated APNs device token metadata from KV |
