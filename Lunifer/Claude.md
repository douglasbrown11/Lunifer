...
- `App.swift` configures Firebase, registers the background sleep-analysis and commute-refresh tasks, purges corrupt legacy sleep-history entries, registers the rest-day notification category, and assigns the app-wide notification delegate on launch. It also installs a top-level `AppDelegate` via `@UIApplicationDelegateAdaptor(AppDelegate.self)` whose `application(_:open:)` forwards URL callbacks to `GIDSignIn.handle` and `Auth.auth().canHandle` — this is **required for Firebase's managed Microsoft OAuth flow** to complete its redirect callback (and it silences the GoogleUtilities `AppDelegateSwizzler` "App Delegate does not conform to UIApplicationDelegate protocol" warning, which was the root cause of the old Microsoft "missing initial state / sessionStorage" failure).
...
- Shows `SleepInsightDetailCard` for the selected bar/period with an On target / Surplus / Debt status pill, the recorded/average duration, and range/night count (aggregate) or bedtime/wake time (individual night). The explicit "Target" and "Above/Below target" detail rows have been removed, though the card still computes `deltaHours` against `recommendedHours` to drive the status pill
...
- Account deletion flow: user taps "Delete Account" → confirmation alert → user taps "Delete" → `performDeletion()` runs. In order: (1) **re-authenticates the user up front** via `reauthenticateCurrentUser(_:)` (see below) — this is done BEFORE any data is removed so `user.delete()` can never fail with `requiresRecentLogin` after data is already wiped (no half-deleted state); (2) cancels all AlarmKit alarms (`LuniferAlarm.shared.cancelAlarm()` + `cancelAllAddedAlarms()`); (3) disconnects WHOOP and Oura from Cloudflare KV via the **awaited** `disconnectAndWait()` (not the fire-and-forget `disconnect()`), so the KV token deletion completes while the session is still valid; (4) best-effort client-side Firestore cleanup of `sleepHistory`, `alarmInferences`, `adaptiveData`, `private`, and the root user document (backup to the server-side `beforeUserDeleted` trigger); (5) `user.delete()`. On success, `clearLocalAccountData()` also calls `GIDSignIn.sharedInstance.signOut()` (so a Google session doesn't linger), wipes all local data, and sets `surveyCompleted = false`, navigating back to the intro screen. If the user cancels the re-auth prompt, the flow aborts cleanly with nothing deleted.
...
- **Poppins** is used for several large numeric/date displays (chosen deliberately over Libre Franklin):
  ...
  - Sleep Insights detail-card date (`point.title`) uses `.custom("Poppins", size: 15).weight(.medium)`.
  - ⚠️ **Poppins `.ttf` files must be added to the `Lunifer/fonts/` bundle and (for safety) registered/bundled like the other fonts** — otherwise these `.custom("Poppins", …)` lookups silently fall back to the system font. Poppins ships static weights; the `.weight(.medium)` usage needs the Medium face present.
