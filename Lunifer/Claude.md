...
- `CalendarChoiceScreen` (in `Screens/Intro/Intro.swift`) appears before sign-in; user picks Apple Calendar / Google Calendar / Outlook / None. Selection stored in `ContentView.preSelectedCalendar` and threaded into both `LuniferSignin` and `LuniferSurvey`.
