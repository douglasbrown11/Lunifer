...
- Notifications screen (`NotificationsSettingsView`) has a master `allNotificationsEnabled` toggle plus four individual toggles: `batteryAlertEnabled` ("Battery Alert"), `wakeReminderEnabled` ("Alarm Set Alert"), `commuteReminderEnabled` ("Commute Reminder"), and `restDayReminderEnabled` ("Rest Day Reminder"). Turning the master off flips all four off and cancels their pending notifications (`WakeNotification`/`CommuteNotification`/`BatteryAlarmNotification`/`RestDayEventNotification`); turning it back on re-enables all four. **Note:** `allNotificationsEnabled` and `restDayReminderEnabled` are raw `@AppStorage` string keys and are NOT yet centralized in `AppPreferencesStore.Keys` (unlike the other three). `RestDayEventNotification.scheduleIfNeeded` gates on both `restDayReminderEnabled` and `allNotificationsEnabled`.
